...
angular.module('myApp.controllers', []).controller('AppCtrl', function ($scope, socket, userService, $interval, $timeout) {
    
    $scope.dataStore = userService.dataStore;
    ...
    socket.on('user:join', function (dataMsg) {
        ...
        console.log(userService.dataStore);
        userService.updateUser(dataMsg);
        console.log(userService.dataStore);
...
}).controller('chatCtrl', function ($scope, socket, userService, $location, $anchorScroll, $timeout) {
    ...
    $scope.pingItToggle = function () {
        ...
        if ((!$scope.ping.running) || ($scope.ping.running === false)) {
            console.log('pingItRunning true');
            $scope.ping.running = true;
            offlineApp.helpers.pingIt.start(socket, $scope);
        } else {
            console.log('pingItRunning false');
